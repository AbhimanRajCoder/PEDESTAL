...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (bgAudio && audioToggle) {
        bgAudio.volume = 0.2; // Keep background music subtle
        let isPlaying = false;

        const setPlayingState = (play) => {
            isPlaying = play;
            if (play) {
                iconMuted.classList.remove('flex');
                iconMuted.classList.add('hidden');
                iconPlaying.classList.remove('hidden');
                iconPlaying.classList.add('flex');
                
                setTimeout(() => {
                    iconMuted.classList.remove('scale-100');
                    iconMuted.classList.add('scale-0');
                    iconPlaying.classList.remove('scale-0');
                    iconPlaying.classList.add('scale-100');
                }, 10);

                audioToggle.classList.remove('text-slate-400', 'border-border');
                audioToggle.classList.add('border-neon-green', 'text-neon-green', 'bg-neon-green/5');
            } else {
                iconPlaying.classList.remove('flex');
                iconPlaying.classList.add('hidden');
                iconMuted.classList.remove('hidden');
                iconMuted.classList.add('flex');
                
                setTimeout(() => {
                    iconPlaying.classList.remove('scale-100');
                    iconPlaying.classList.add('scale-0');
                    iconMuted.classList.remove('scale-0');
                    iconMuted.classList.add('scale-100');
                }, 10);

                audioToggle.classList.remove('border-neon-green', 'text-neon-green', 'bg-neon-green/5');
                audioToggle.classList.add('text-slate-400', 'border-border');
            }
        };
        ...
        if (playPromise !== undefined) {
            playPromise.then(() => {
                ...
                // Fallback: Start music on the user's very first interaction with the page
                const startOnInteraction = () => {
                    if (!isPlaying) {
                        bgAudio.play().then(() => setPlayingState(true)).catch(e => console.log(e));
                    }
